...
function fc_get_date_options( $json_encode=true, $for_php_date = false ){
	...
	foreach($map as $name => $opt){
		...
		if( $for_php_date ){
			...
			$value = preg_replace('/((?<!\\\\)[dDjlNSwzWFmMntLoYyaABgGhHisueIOPTZcrUvV])/', '\\\\$1', $value );
		}
...
if( !function_exists('is_rhc_event') ):
function is_rhc_event(){
...
if( !function_exists('is_rhc_tax') ):
function is_rhc_tax(){
	global $rhc_plugin;
	if( $rhc_plugin->template_frontend->is_taxonomy ) return true;
	$o = get_queried_object();
	//----
