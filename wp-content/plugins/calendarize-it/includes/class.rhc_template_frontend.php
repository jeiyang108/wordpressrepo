...
class rhc_template_frontend {
	...
	function validate_request_event_rdate( $id ){
		global $wpdb,$rhc_plugin;
		if( '1' == $rhc_plugin->get_option( 'validate_event_rdate', '1', true) ){
			if( isset( $_REQUEST['event_rdate'] ) ){
				...
				if(count($arr)==2){
					...
					$sql = "SELECT COUNT(*) FROM {$wpdb->prefix}rhc_events WHERE post_id=%s AND event_start='%s'";
					$sql = sprintf($sql,
						intval( $id ),
						date('Y-m-d H:i:s', $start)
					);
					
					if( 0 == intval( $wpdb->get_var( $sql, 0, 0 ) ) ){
						unset( $_REQUEST['event_rdate'] );
					}
				}
			}		
		}
	...
	function handle_main_query( $wp ){
		...
		if(is_single() && $o->post_type==RHC_EVENTS){
			$this->is_event = true;
			$this->post_ID = $o->ID;
			$this->validate_request_event_rdate( $o->ID );
			...
			$this->event_template_id = $template_page_id;
...
?><div itemscope="itemscope" itemtype="http://schema.org/Event"><?php
		do_action( 'rhc_inside_schema_event', $p, $arg_post, $echo, $next_upcoming );
?><meta itemprop="name" content="<?php echo get_the_title( $p->ID) ?>"><?php	
...
?><meta itemprop="endDate" content="<?php echo esc_attr( $end ) ?>"><?php		
		...
		if( apply_filters( 'rhc_event_microdata', false, $p ) ){
...
?><div <?php if($inside_event):?>itemprop="organizer"<?php endif;?> itemscope="itemscope" itemtype="http://schema.org/Organization"><?php
...
?><div <?php if($inside_event):?>itemprop="location"<?php endif;?> itemscope="itemscope" itemtype="http://schema.org/Place"><?php
...
?><meta itemprop="hasMap" content="http://maps.google.com/?q=<?php echo urlencode($gaddress)?>" /><?php				
				...
				if( empty( $address ) && !empty( $gaddress ) ){
					//--use google address as street address
					$address = $gaddress;
				}
				
				if( !empty($address) || !empty($locality) || !empty($region) || !empty($zip) ){
					//Place::adress::PostalAddress
?><div itemprop="address" itemscope="itemscope" itemtype="http://schema.org/PostalAddress"><?php
...
?><div itemprop="geo" itemscope="itemscope" itemtype="http://schema.org/GeoCoordinates"><?php
...
*/
	...
	function get_taxonomy_content($_term_id,$_taxonomy,$wrap=''){
		...
		if($taxonomy==RHC_VENUE){
			$this->venue_microdata( $term, true );
		}
