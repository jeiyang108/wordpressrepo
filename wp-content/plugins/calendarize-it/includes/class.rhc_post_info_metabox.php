		...
		if ( 'auto-draft' == $post->post_status && $this->post_type == $post->post_type ) {
			if ( $post_ID > 0 ) {
				if ( current_user_can( $this->capability, $post_ID ) ) {
					...
					rhc_post_info_shortcode::save( $post_ID, $postinfo_boxes );
				}			
			}			
		}
	}
	...
	function wp_ajax_update_postinfo() {
		...
		if ( $post_ID > 0 ) {
			if ( current_user_can( $this->capability, $post_ID ) ) {
				...
				$postinfo_boxes = is_array( $postinfo_boxes ) ? $postinfo_boxes : array();
				$postinfo_boxes[ $postinfo_boxes_id ] = (object) array(
	...
	function wp_ajax_pinfo_reset_to_default() {
		...
		if ( ! current_user_can( 'edit_' . $this->post_type, $post_ID ) ) {
			die( json_encode( (object) array( 
				'R'   => 'ERR',
				'MSG' => __( 'You cannot edit this post.','rhc' ) 
			) ) );
		}
